...
function handleDesignViewDrop(componentRec)
{
	...
	if (componentRec)
	{
      ...
      {
		var tableName = componentRec.getName();
		//if (dw.getDocumentDOM().serverModel.getServerName() == "ASP") {
			tableName = encodeSQLReference(tableName, MM.recordsetSBDefaults.connectionName);
		//}
        MM.recordsetSBDefaults.sql = "SELECT * FROM " + tableName;
      }

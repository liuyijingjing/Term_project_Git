 ...
 _RecordsetName.initializeUI();
  ...
  if( DOCTYPE_IS_CFC ) 
  {
	...
	var strEnteredValue = dw.loadString("recordset/coldfusion/paramater_type/entered_value")
  	_FilterParameter.initializeUI(new Array(strFunctionArgument, strEnteredValue), new Array("Function Argument", strEnteredValue));
  }
  else 
  {
  ...
  if (sqlObject.getType() == SQLStatement.STMT_TYPE_SELECT)
  {
    ...
    if (filterInfo != null)
    {
      ...
      if (filterInfo.rval)
      {
        ...
        if (paramInfo.paramType == dw.loadString('Startup/MMinit/MM.LABEL_CF_Param_Types_5') && sqlParams.length > 0)
        {
...
function updateSBRecordsetObject()
{
	...
	if (_FilterColumn.getValue() != "") 
	{
		...
		//   where clause.
		if (paramInfo != null)    {
			var paramName = paramInfo.runtimeVal;
			
			var serverObj = dwscripts.getServerImplObject();
			if (serverObj != null && serverObj.getSQLStringForDBColumnType != null) {
				paramName = dwscripts.getSQLStringForDBColumnType(paramName, _FilterColumn.getType());
			}
			
			RECORDSET_SBOBJ.addSimpleWhere(
								sqlObject,
								_FilterColumn.getValue(),
								_FilterOperator.getValue(),
								paramName,
								getOldParamObj(RECORDSET_SBOBJ, paramName)
			);
			  
			// Get filter column properties
//			var connectionName = RECORDSET_SBOBJ.applyParameters.ConnectionName;
//			var tableName = sqlObject.fromClause;
//			var columnName = _FilterColumn.getValue();
//			var fieldObj = FieldTypes.getFieldObj(connectionName, tableName, columnName);

			var sqlParam = new Object();
			sqlParam.varName = paramInfo.nameVal;
//			sqlParam.varType = FieldTypes.getFieldTypeFromObj(fieldObj);
		...
		{
			RECORDSET_SBOBJ.addSimpleWhere(sqlObject, _FilterColumn.getValue(), _FilterOperator.getValue(), _FilterParameterValue.getValue(), undefined, true, _FilterColumn.getType());      
		}
	}

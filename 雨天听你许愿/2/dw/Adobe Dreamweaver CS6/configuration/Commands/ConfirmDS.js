...
function render() {
   ...
   if (arguments.length == 3){
     ...
     for (i=1; i<arguments.length; i++){
       btnList += "<input type='button' value='"+arguments[i]+"' onClick='setResult(\""+arguments[i]+"\")' class='yesNoOkCancel'> ";
     }
...
function render_array(message, buttons) {
   ...
   document.msg.innerHTML = message; //Insert the question
     for (i=0; i<buttons.length; i++)
       btnList += "<input type='button' value='"+buttons[i]+"' onClick='setResult(\""+buttons[i]+"\")' class='yesNoOkCancel'> ";

...
function canInsertObject()
{
  ...
  if (!errMsgStr)
  {
    ...
    var editOpType2 = (EDIT_OP_TYPE == "Insert") ? "insertRecord2" : "updateRecord2";
    ...
    {
      found = ((ssRecs[i].type == editOpType) || (ssRecs[i].type == editOpType2));
    }
...
//  var editOpsGroup = new Group( (editOpTypeIsInsert)?"insertRecord2":"updateRecord" ); 
...
//  customGroup.addParticipants(Array(formPart));
  ...
  paramObj.ext = getServerExtension();
	...
	*/
	...
	var elements = GRID_COLS.valueList;

	var columns = new Array();
	var values = new Array();
	var updateSet = new Array();
	var tempValue, tempObj;
	...
	for (var i=0; i < elements.length; i++) {
		if (elements[i].column && elements[i].displayAs && (elements[i].displayAs.type != "text")) {
			tempObj = new Object();
			tempObj.formControl = elements[i].fieldName;
			tempObj.trueValue = "";
			tempObj.falseValue = "";
			hasIIF = false;
			if (checkboxesHash[lookupTable[elements[i].submitAs]]) {
				tempObj.trueValue = checkboxesHash[lookupTable[elements[i].submitAs]].split("/")[0];
				tempObj.falseValue = checkboxesHash[lookupTable[elements[i].submitAs]].split("/")[1];
				paramObj.needIIFInPage = true;
				hasIIF = true;
			}
			tempValue = extPart.getInsertString("", "parameterValue", tempObj);

			columns.push(/*wrapNamesWithSpaces*/FieldTypes.encodeSQLReference(elements[i].column, paramObj.cname));
			values.push("?");
			updateSet.push(/*wrapNamesWithSpaces*/FieldTypes.encodeSQLReference(elements[i].column, paramObj.cname) + " = ?");

			tempType = FieldTypes.castDBType(dwscripts.getDBColumnTypeEnum(oldTypeToNewHash[lookupTable[elements[i].submitAs]]), true);
	...
	if (EDIT_OP_TYPE == "Insert") {
		...
		FieldTypes.addParameter(paramObj, tempType, tempSize, tempValue, true);
  ...
  //customGroup.apply(paramObj);
	var groupName = (EDIT_OP_TYPE == "Insert") ? "insertRecordCmd2" : "updateRecordCmd2";
...
function setFieldsAndColumns(paramObj) {
  ...
  for (i=0;i<nCols;i++){
    ...
    if (currObj.displayAs.type != "text"){ // if a form element
      ...
      columnsStr += /*wrapNamesWithSpaces*/FieldTypes.encodeSQLReference(currObj.column, paramObj.cname) + "|" + lookupTable[submitType] + "|";
    }
  }

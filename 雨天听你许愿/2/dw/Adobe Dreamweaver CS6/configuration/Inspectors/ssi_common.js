...
function editInclude()
{
	...
	if ( absoluteURL != "" )
	{
	    if (FoundInRelatedFiles(absoluteURL))
	    {
	        // If this SSI is detected as RelatedFile, open as RelatedFile.
	        dreamweaver.openRelatedFile(absoluteURL);
	    }
	    else
	    {   
	        // If it's non-related doc. Or, if the RelatedFile is turned OFF,
	        // then, open as a traditional way. (top doc).s
...
function virtualToFile ( relativeURL )
{
  ...
  docRelURL = dw.absoluteURLToDocRelative( docPathURL, siteRootURL, absoluteURL );
...
function FoundInRelatedFiles(filename){    var found = false;	// Get the list of RelatedFiles	UseMenuNames = false;	var docList = dw.getRelatedFiles(UseMenuNames);  	if (docList.length > 0)	{	  relatedFiles = new Array(docList.length);		for (i = 0; i < docList.length; i++)		{
			// Check to see if the file is one of the 
			// RelatedFiles in the parent file.			if (docList[i] == filename)			    found = true;	// matched the whole URL		}	}	return found;}
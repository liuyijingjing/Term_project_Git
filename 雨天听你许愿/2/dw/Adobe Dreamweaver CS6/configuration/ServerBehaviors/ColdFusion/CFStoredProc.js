...
var SB_FILE = dw.getConfigurationPath() + "/ServerBehaviors/ColdFusion/CFStoredProc.htm";
...
function updateUI(control, event)
{
  ...
  else if (control == "EditParam")
  {
    ...
    cmdArgs[3] = new Object;
	cmdArgs[3].varType = "disable";
	cmdArgs[3].defaultVal = PARAM_LIST.getValue();
    var editParamResult = dwscripts.callCommand("Edit CF Parameter", cmdArgs);            
    if (editParamResult && editParamResult.length)
    {
      PARAM_LIST.setValue(editParamResult[1].defaultVal);
      updateParamProperties(); 
      updateParamEditButtonState(); 
    }
  }
...
function onAddParameter()
{
  ...
  if (addParamResult && addParamResult.length && addParamResult[0])
  {
    ...
    if (indexOfParam != -1)
    {
      ...
      {
        PARAM_LIST.setValue(addParamResult[1].defaultVal, indexOfParam);
      }
    }
    else
    {
      PARAM_LIST.append(addParamResult[0],addParamResult[1].defaultVal);
    }
    updateParamProperties(); 
    updateParamEditButtonState(); 
  }
...
function applyServerBehavior(priorSBStoredProc)
{
  ...
  if (CONNECTION_NAME_FIELD.listControl.getLen() > 2)
  {
    dwscripts.saveExtensionData(SB_FILE,"lastConnection",CONNECTION_NAME_FIELD.getValue());
  }

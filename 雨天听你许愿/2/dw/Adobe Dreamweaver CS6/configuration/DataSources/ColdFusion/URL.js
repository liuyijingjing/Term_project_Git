...
function addDynamicSource()
{
  ...
  if (MM.retVal == "OK")
  {
    ...
    if (theResponse.length)
    {
      ...
      if (siteURL.length)
      {
        dwscripts.addListValueToNote(siteURL, "URL", theResponse, false, true);   
      }
      else
      {
...
function generateDynamicSourceBindings(sourceName)
{
  ...
  if (siteURL.length)
  {
    ...
    dw.codeHints.resetMenu("CodeHints_Object_Methods", "URL.",
	 "ColdFusion");
	if (bindingsArray.length > 0)
      dw.codeHints.addMenu("CodeHints_Object_Methods","URL.",

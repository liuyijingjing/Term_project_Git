...
function addDynamicSource()
{
  ...
  if (MM.retVal == "OK")
  {
    ...
    if (theResponse.length)
    {
      ...
      if (siteURL.length)
      {
        dwscripts.addListValueToNote(siteURL, "Form", theResponse, false, true);   
      }
      else
      {
...
function findDynamicSources()
{
  ...
  if (siteURL.length)
  {
    ...
    dw.codeHints.resetMenu("CodeHints_Object_Methods", "Form.",
	 "ColdFusion");
	if (bindingsArray.length > 0)
      dw.codeHints.addMenu("CodeHints_Object_Methods","Form.",

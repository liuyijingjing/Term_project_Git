...
function applyBehavior()
{
	...
	for (i = 0; i < imgSrcArray.length; i++)
	{
		...
		if (imgSrc)
		{
			...
			if (argList)
				argList += ",";    //if stuff already in list, add comma

			imgObjNS = document.MM_NS_REFS[i]; //get NS ref

			if (imgObjNS.indexOf(".") == (-1) || endsWithAnyNum.test(imgObjNS))
			{
				//found rename item, prompt user & fixup

				newName = "";  //may need a new or unique name

				if (imgObjNS.indexOf(REF_UNNAMED) == 0)
				{
					//if the image is unnamed
	
					newName = getUniqueName("IMG", "Image", imgObjsArray);    //generate unique name
				}
				else if (proceed == null)
				{
					//if first dupe, prompt user if the want renames
					
					if (document.SILENT_MODE)
					{
						proceed = true;
					}
					else
					{
						...
						if (!proceed)
							return "";  //cancel applyBehavior
					}
				}
	
				if (endsWithAnyNum.test(imgObjNS))
				{
					//if ends with number > 0: foo[1], use name gen new one
				...
				if (newName)
				{
					//if the image should be renamed
			...
			if (preload)
				preloadArray.push(dw.doURLEncoding(imgSrc)); //add string to list
		}
	}

	if (!argList)
	{
		retVal = MSG_NoImgsSrc;
	}
	else
	{
		//OK
		...
		{
			if (document.theForm.restore.checked)
			{
				//add restore to onMouseOut handler
		...
		if (OLD_PRELOAD_ID && OLD_PRELOAD_ARRAY)
		{
			//if previously preloaded, remove DW2 preloads

			if (OLD_PRELOAD_ID.toString().indexOf("#") != -1)
			{
				//DW2 preload call, remove it
